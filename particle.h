...
public:
    ...
    PS::S32 ku = 0;
    PS::F64 tmp_r_planet = 0.;
    PS::F64 pre_r_planet = 0.;
    PS::F64 init_mass = 0.;
    PS::F64 pre_mass = 0.;
    PS::F64 tmp_mass = 0.;
    PS::F64 tmp_tar_mass = 0.;
    PS::F64 tmp_imp_mass = 0.;
    PS::F64 delta_mass = 0.;
    PS::F64 delta_mass_tar = 0.0;
    PS::F64 delta_mass_imp = 0.0;
    PS::F64 s_type = 0.;
    PS::S32 tar_flag = 0;
    PS::S32 imp_flag = 0;
    PS::S32 col_flag = 0;
...
*/
    ...
    std::string setfilename(std::string name, PS::S32 i){
            std::string num = std::to_string(i+1);
            name = name + num + ".dat";
            return name;
    }

    PS::F64 interp1dim(std::string a, PS::S32 k){
            std::ifstream ifs;
            ifs.open(setfilename(a,k));
            if(ifs.fail()){
...
/*
    void fileNumber(){
         ...
         PS::F64 once = log(0.1);
         for(PS::S32 i=0; i<691; i++){
           if( (jor*exp(once + 0.01*i)<=init_mass) && (init_mass<jor*exp(once + 0.01*(i+1))) ){
                ku = i;
                break;
           }
           if( init_mass < jor*0.1 ){
                ku = 0;
                break;
           }
           if( jor*exp(once + 0.01*691) <= init_mass ){
                ku = 691;
                break;
           }
         }
...
*/ 
    void setRPlanet(){
           init_mass = mass;
           PS::F64 x = 0.;
           std::string filename = "10000star";
           PS::F64 once = log(0.1);
           const PS::F64 jor = 0.000248671;
           for(PS::S32 i=0; i<691; i++){
                if( (jor*exp(once + 0.01*i)<=init_mass) && (init_mass<jor*exp(once + 0.01*(i+1))) ){
                      x = interp1dim(filename,i);
                      ku = i;
                      break;
                }
                if( init_mass < jor*0.1 ){
                      x = interp1dim(filename, 0);
                      ku = 0;
                      break;
                }
                if( jor*exp(once + 0.01*691) <=init_mass ){
                      x = interp1dim(filename,691);
                      ku = 691;
                      break;
                }
            }
...
#endif
        if ( !fprintf(fp, "%lld\t%20.15e\t%d\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%20.15e\t%d\t%d\n",
                      this->id, this->s_type, this->ku, this->mass, this->r_planet, this->f, this->eccvalue,  
                      this->pos.x, this->pos.y, this->pos.z,
                      this->vel.x, this->vel.y, this->vel.z,
                      this->neighbor.number, Flag) ) {
            //this->r_out, this->r_search) ){
...
#pragma omp parallel for reduction (max: r_max, r_min)
    ...
    std::cerr << __FUNCTION__ << " LINE:" << __LINE__ << " " << PS::Comm::getRank() << std::endl;

    for(PS::S32 i=0; i<N; i++){
        v_ave_glb[i] = PS::Comm::getSum(v_ave_loc[i]);
    }
